...
async function getCurrentPage(): Promise<CurrentPage | undefined> {
    ...
    const pageContent = await chrome.tabs.sendMessage(currentTab[0].id, {
        type: MessageType.GET_PAGE_CONTENTS,
    });
...
chrome.runtime.onMessage.addListener(function (message: Message, _, sendResponse) {
    switch (message.type) {
        ...
        case MessageType.GET_AUTH_TOKEN:
            if (waitingForToken.value == false) getAuthTokenFromStorage().then(sendResponse);
            else watch(waitingForToken, () => getAuthTokenFromStorage().then(sendResponse));
            break;
